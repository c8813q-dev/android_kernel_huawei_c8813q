...
/*define the upgrade fw and conifg */
...
struct cyttsp4_loader_data {
	...
	struct completion builtin_bin_fw_complete; //use to upgrade fw by bin file
	int builtin_bin_fw_status; //status
#endif
	...
	u8 *config_data; //point the upgrade condig date
	int config_size; 
	bool config_loading;
#endif
};
		...
		u32 fw_ver_new, u32 fw_revctrl_new_h, u32 fw_revctrl_new_l)
{
	...
	if (fw_ver_new < fw_ver_img) {
		dev_dbg(dev, "%s: Image is older, will upgrade\n",
				__func__);
		return -1;
	}
...
/*change the function of calibrate*/
...
/*get the version of the bin file*/
