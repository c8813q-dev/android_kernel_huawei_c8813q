	...
	const struct i2c_device_id *i2c_id)
{
	...
	{
	    rc = gpio_request(GPIO_TP_1V8, "tp_1v8");
	    if(rc)
	    {
			dev_err(dev, "%s: Fail to request rdwr gpio PIN.\n", __func__);
			return 0;
	    }
	    gpio_tlmm_config(GPIO_CFG(GPIO_TP_1V8, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	    rc = gpio_direction_output(GPIO_TP_1V8,1);
	}
